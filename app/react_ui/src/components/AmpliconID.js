...
export function Identify({
  ...
  const [keyBiosamples, setkeyBiosamples] = useState([]);
  ...
  const [datarings, setdatarings] = useState([
    {
      ...
      feature: "PATHOVAR", // any valid column name in CSV file, or property of row object
      height: 15,
      padding: 5,
      color: {
  ...
  useEffect(() => {
    // set phylogenetic and functional plotting data when keyBiosamples changes
    let PhylogroupData = {};
    let SpeciesData = {};
    let PathovarData = {};
    let T3SSData = {};
    let T3EData = {};
    let WHOPData = {};
    keyBiosamples.forEach((ID) => {
      let relative = metadata.get(ID);
      if (relative.PG in PhylogroupData) {
        PhylogroupData[relative.PG]++;
      } else {
        PhylogroupData[relative.PG] = 1;
      }
    ...
    if (typeof selectedQuery[0].distances !== "undefined") {
      selectedQuery[0].distances.forEach((ref) => {
        ...
        if (similarity >= rangeValue) {
          ...
          BiosampleNames.push(ref.name.replaceAll("^", ""));
        }
      });
      console.log(nodeNames)
      setkeyNodes(nodeNames);
      setkeyBiosamples(BiosampleNames);
    ...
    <>
      ...
      >
        ...
        <p style={{ textAlign: "center", fontSize: "24pt" }}>
          <b>{keyBiosamples.length} reference strains found with ANI {'≥ ' + rangeUpdate + '%' } </b>
        </p>
        <Divider />
        <p style={{ textAlign: "left", fontSize: "18pt" }}>
        ...
        <Grid>
          <Grid.Column width={4} textAlign="center" marginLeft="100px"><img src={T3SS_IMG} alt="Logo" height="250px" margin="auto" /><p
              style={{
                textAlign: "center",
                fontSize: "12pt",
                marginTop: "1vh",
            ...
            >
              <b>Canonical type 3 Secretion system</b> <br/>carried by close relatives
            </p></Grid.Column>
          <Grid.Column width={12}>
            <div style={{ height: "220px", width: "75%", margin: "auto" , marginTop:"50px", marginLeft:"0px"}}>
              <ResponsiveBar
                data={T3SSBar}
                keys={["value"]}
                indexBy="id"
                margin={{ left: 30, right: 20, top: 10, bottom: 40 }}
                padding={0.1}
                valueScale={{ type: "linear" }}
                indexScale={{ type: "band", round: true }}
                colors={"#366c7d"}
        ...
        <Divider />
        <Grid>
        <Grid.Column width={4} textAlign="center" marginLeft="100px"><img src={T3E_IMG} alt="Logo" height="250px" margin="auto" /><p
              style={{
                textAlign: "center",
                fontSize: "12pt",
                marginTop: "1vh",
            ...
            >
              <b>Type 3 effector proteins</b> <br/>carried by close relatives
            </p></Grid.Column>
          <Grid.Column width={12}>
            <div style={{ height: "220px", width: "75%", margin: "auto" , marginTop:"50px", marginLeft:"0px"}}>
              <ResponsiveBar
                ...
                keys={["value"]}
                indexBy="id"
                margin={{ left: 30, right: 20, top: 10, bottom: 60 }}
                padding={0.1}
                valueScale={{ type: "linear" }}
                indexScale={{ type: "band", round: true }}
                colors={"#7d3636"}
        ...
        <Divider />
        <Grid>
        <Grid.Column width={4} textAlign="center" marginLeft="100px"><img src={WHOP_IMG} alt="Logo" height="250px" margin="auto" /><p
              style={{
                textAlign: "center",
                fontSize: "12pt",
                marginTop: "1vh",
            ...
            >
              <b>WHOP genes</b> <br/>carried by close relatives:
            </p></Grid.Column>
          <Grid.Column width={12}>
            <div style={{ height: "220px", width: "75%", margin: "auto" , marginTop:"50px", marginLeft:"0px"}}>
              <ResponsiveBar
